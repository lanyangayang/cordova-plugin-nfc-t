...
public class Util {
    ...
    static JSONObject ndefToJSON(Ndef ndef) {
        ...
        if (ndef != null) {
            try {
                ...
                Tag tag = ndef.getTag();
                // 在 NfcUtil.parseMessage 被重构之前，对于 NDEF_FORMATABLE，tag 将为 null
                if (tag != null) {
                ...
                // 解决 ICS（Android 4.0 和 4.0.1）中的一个 bug，
                // 即 Ndef 对象的 mTag.getTagService(); 有时返回 null
                // 参见 http://issues.mroland.at/index.php?do=details&task_id=47
                try {
                ...
                Log.e(TAG, "将 ndef 转换为 json 失败：" + ndef.toString(), e);
            }
        }
        return json;
    }
    ...
    static JSONObject tagToJSON(Tag tag) {
        ...
        if (tag != null) {
            try {
                ...
                Log.e(TAG, "将 tag 转换为 json 失败：" + tag.toString(), e);
            }
        }
        return json;
    }
    ...
    static JSONObject recordToJSON(NdefRecord record) {
        ...
        try {
            ...
            // 不确定为什么会发生这种情况，文档描述不明确。
            Log.e(TAG, "将 ndef 记录转换为 json 失败：" + record.toString(), e);
        }
        return json;
    }
